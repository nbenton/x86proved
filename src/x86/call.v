...
Require Import x86proved.spred x86proved.spectac x86proved.spec x86proved.safe x86proved.cursor x86proved.x86.instr.
...
Definition toyfun f P Q :=
  Forall iret,
  safe @ (UIP~=iret ** retreg? ** Q)
      -->> safe @ (UIP~=f ** retreg~=iret ** P).
...
Proof.
  ...
  cancel2. cancel1. ssimpl. autorewrite with push_at. cancel1. by ssimpl.  
Qed. 

Global Opaque call_toyfun.
Global Instance: forall f : ADDR, instrrule (call_toyfun f) := @toyfun_call.

Lemma toyfun_mkbody (f f': ADDR) P p Q:
...
Example toyfun_example_callee_correct_helper (f f': ADDR):
...
Example toyfun_example_correct entry (i j: ADDR) a:
  |-- (
      safe @ (UIP ~= j ** EAX ~= a +# 4) -->>
          safe @ (UIP ~= entry ** EAX ~= a)
...
Example toyfun_apply_correct (f f' g: ADDR) P Q:
  |-- (
      toyfun g (P ** RBX?) Q -->> toyfun f (P ** RBX ~= g) Q
    ) c@ (f--f' :-> toyfun_apply).
Proof.
