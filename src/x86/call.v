...
Lemma loopy_toyfun_call (f:ADDR) P (O:PointedOPred) Q:
...
Lemma toyfun_call (f:ADDR) P (O:OPred) Q:
...
Global Instance: forall f : ADDR, loopy_instrrule (call_toyfun f) := @loopy_toyfun_call.
Global Instance: forall f : ADDR, instrrule (call_toyfun f) := @toyfun_call.

Lemma toyfun_mkbody {T_OPred proj} (f f': ADDR) P p (O : OPred) Q:
...
Definition toyfun_example (entry: ADDR) : program :=
  LOCAL f;
  f:;;
...
Example toyfun_example_callee_correct_helper {T_OPred proj} mkCatOP mkEmpOP (f f': ADDR):
...
Definition toyfun_example_callee_correct (f f': ADDR):
...
Definition loopy_toyfun_example_callee_correct (f f': ADDR):
...
Example toyfun_example_caller_correct a (f:ADDR):
  Forall a', toyfun f (EAX ~= a') empOP (EAX ~= a' +# 2)
...
Example loopy_toyfun_example_caller_correct a (f:ADDR):
  Forall a', loopy_toyfun f (EAX ~= a') empOP (EAX ~= a' +# 2)
...
Example toyfun_example_correct entry (i j: ADDR) a (O : OPred):
  |-- (
      obs O @ (UIP ~= j ** EAX ~= a +# 4) -->>
          obs O @ (UIP ~= entry ** EAX ~= a)
...
Example toyfun_apply_correct (f f' g: ADDR) P O Q:
  |-- (
      toyfun g (P ** RBX?) O Q -->> toyfun f (P ** RBX ~= g) O Q
    ) <@ (f--f' :-> toyfun_apply).
Proof.
