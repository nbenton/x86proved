...
Require Import x86proved.spred x86proved.spec x86proved.x86.basic x86proved.x86.program x86proved.spectac.
...
Lemma JCC_rule a cc cv (b:bool) (p q: ADDR) :
  |-- ((|> safe @ (b == cv /\\ UIP ~= a ** ConditionIs cc b)) //\\
         safe @ (b == (~~cv) /\\ UIP ~= q ** ConditionIs cc b) -->>
...
Lemma JZ_rule a (b:bool) (p q: ADDR) :
  |-- (
      |> safe @ (b == true  /\\ UIP ~= a ** ZF ~= b) //\\
         safe @ (b == false /\\ UIP ~= q ** ZF ~= b) -->>
      safe @ (UIP ~= p ** ZF ~= b)
    ) c@ (p -- q :-> JZ a).
Proof.
...
Lemma JC_rule a (b:bool) (p q: ADDR) :
  |-- (|> safe @ (b == true  /\\ UIP ~= a ** CF ~= b) //\\
          safe @ (b == false /\\ UIP ~= q ** CF ~= b) -->>
      safe @ (UIP ~= p ** CF ~= b)
    ) c@ (p -- q :-> JC a).
Proof.
...
Lemma JMP_R_rule (r:GPReg64) addr (p q: ADDR) :
  |-- (|> safe @ (UIP ~= addr ** r ~= addr) -->> safe @ (UIP ~= p ** r ~= addr)) c@
        (p -- q :-> JMP r).
...
Lemma CALL_I_rule (a:ADDR) (p q: ADDR) :
  |-- Forall w: ADDR, Forall sp:ADDR, (
      |> safe @ (UIP ~= a ** USP~=sp-#8 ** sp-#8 :-> q) -->>
...
(*=End *)
  ...
  Proof.
    ...
    (* We need to set up Lob induction but not for this instruction. This is a
       bit awkward. *)
    assert (|> safe @ (UIP ~= test ** P) -->>
        safe @ (UIP~=i ** P) //\\ safe @ (UIP ~= test ** P) |--
            safe @ (UIP~=i ** P)) as Hlob.
    - etransitivity; [|by apply landL1].
      instantiate (1 := safe @ (UIP ~= test ** P)).
