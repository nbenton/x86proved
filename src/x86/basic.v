...
Require Import x86proved.spred x86proved.spec x86proved.spectac x86proved.safe x86proved.cursor x86proved.reader x86proved.common_tactics.
...
Section Basic.
  ...
  Definition basic P (c : T) Q : spec :=
    Forall (i j : ADDR),
    (safe @ (UIP ~= j ** Q) -->> safe @ (UIP ~= i ** P)) @ (i -- j :-> c).
