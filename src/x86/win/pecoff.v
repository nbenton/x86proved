...
Definition IMAGE_FILE_MACHINE_AMD64 := #x"8664".
...
Definition makeMinimalHeader (isX64:bool) (targetType : TargetType) numberOfSections opthdrsize :=
  makeCOFFFileHeader
    (if isX64 then IMAGE_FILE_MACHINE_AMD64 else IMAGE_FILE_MACHINE_i386)
    numberOfSections (* Number of sections *)
    0             (* Timestamp *)
    0             (* Symbol table *)
    0              (* Number of symbols *)
    opthdrsize     (* Size of optional header *)
    ((if isX64 then IMAGE_FILE_LARGE_ADDRESS_AWARE else IMAGE_FILE_32BIT_MACHINE) ||
...
Definition fileAlign: DWORD := iter fileAlignBits (shlB (n:=n32)) #1.

(* Once mapped into memory the sections must be on 4K boundaries *)
Definition sectAlignBits := 12.
Definition sectAlign: DWORD := iter sectAlignBits (shlB (n:=n32)) #1.
 ...
 *)
Fixpoint computeExportTables (dRVA:ADDR -> program) (exports: seq DLLExport) baseEAT baseENPT baseEOT (kEAT kENPT kEOT : program) (ord : WORD) : program :=
...
Definition makeExportDirectoryTable (dRVA:ADDR -> program) (nbEntries:DWORD) (baseName baseEAT baseENPT baseEOT : ADDR) :=
  dd 0;; (* Reserved *)
  dd 0;; (* Time stamp *)
  dw 0;; (* Major version *)
  dw 0;; (* Minor version *)
...
(* We lay out the .idata section as follows.
     ...
     IAT (One ADDR per entry, with null at end, for each DLL)
  ...
  match imports, importAddrs with
   (IATbase,(ILTbase,Build_DLLImport _ entries))::imports, addrs::importAddrs =>
    (fix computeIATandILTforOneDLL (entries: seq ImportEntry) (addrs: seq ADDR) (IAT ILT: program) : program :=
       ...
       | _, _ => makeIATsILTs isX64 dRVA imports startIAT endIAT importAddrs 
                 (IAT;; if isX64 then dq 0 else dd 0) (ILT;; if isX64 then dq 0 else dd 0)
      end
    ) entries addrs (IATs;; align (if isX64 then 3 else 2);; IATbase:;) (ILTs;; align (if isX64 then 3 else 2);; ILTbase:;)
  | _, _ => startIAT:;; IATs;; endIAT:;; ILTs
  end.
...
Fixpoint makeImportDirectory (dRVA:ADDR -> program) (imports: seq (ADDR*(ADDR*DLLImport))) endIDT :=
...
Definition makePEfile
  ...
  IATStart IATEnd : ADDR)
  (sections: seq section) :=
let ddq (d:DWORD) := if isX64 then dq (zeroExtend 32 d) else dd d in
LOCAL imageBase;
...
imageBase:;;
  MSDOSStub;;
  PEsig;;
  makeMinimalHeader isX64 targetType (size sections) (low 16 (endOptionalHeader - startOptionalHeader));;
...
startOptionalHeader:;;
  dw (if isX64 then #x"020B" else #x"010B");;    (* PE32 or PE32+ format *)
  db 11;; db 0;; (* linker version, major & minor numbers *)
  dd SizeOfCode;;
  dd SizeOfInitializedData;;
  dd SizeOfUninitializedData;;
  dRVA (if entryPoint == (0:ADDR) then if targetType is EXE then zeroExtend 32 BaseOfCode else 0:ADDR else entryPoint);;
  dRVA (zeroExtend _ BaseOfCode);;
  (if isX64 then prog_skip else dRVA (zeroExtend _ BaseOfData));;
  (if isX64 then dq imageBase else dd (low 32 imageBase));;
  ...
  ddq #x"00001000";;               (* SizeOfStackCommit *)
  ddq #x"00100000";;               (* SizeOfHeapReserve *)
  ddq #x"00001000";;               (* SizeOfHeapCommit *)
  dd 0;;                          (* LoaderFlags, must be zero *)
  dd 16;;                         (* NumberOfRvaAndSizes *)
startDirectories:;;
  (* Exports *)
  dRVA exportTableStart;; dd (low 32 (exportTableEnd - exportTableStart));;
  (* Imports *)
  dRVA importTableStart;; dd (low 32 (importTableEnd - importTableStart));;
  (* Resources *)
  dd 0;; dd 0;;
  (* Exceptions *)
  dd 0;; dd 0;;
...
Definition makeExportSection (dRVA:ADDR -> program) startEDT endEDT progName exports :=
...
Definition makeImportSection (isX64:bool) dRVA startIDT endIDT startIAT endIAT imports (importAddrs: seq (seq ADDR)) :=
  (* Labels for IATs *)
  ...
  makeIATsILTs isX64 dRVA imports2 startIAT endIAT importAddrs prog_skip prog_skip)).
...
(*=topprog *)
Inductive topprog :=
| topprog_global (name: option string) (t: ADDR -> topprog)
| topprog_importdll (name: string) (p: topimps)
| topprog_entry (t: ADDR -> topprog)
| topprog_sect (sect: section) (t: topprog)
| topprog_end
with topimps :=
| topimps_import (name: string) (t: ADDR -> topimps)
| topimps_topprog (t: topprog).

Coercion topimps_topprog : topprog >-> topimps.

Fixpoint applyTopProg {T} (local: (ADDR -> T) -> T) k (imports: seq DLLImport) (importAddrs: seq (seq ADDR))
  (exports: seq DLLExport) (sections: seq section) (entry:ADDR) t : T :=
  match t with
  | topprog_sect s t =>
...
with applyTopImps {T} (local : (ADDR -> T) -> T) k nameDLL entries addrs imports importAddrs exports sections entry (t: topimps) :=
  match t with
...
(*=computeRVAsIn *)
Definition computeRVAsIn (f: (ADDR -> program) -> program) : program :=
  LOCAL base;
  base:;;
    f (fun VA => dd (if VA == #0 then #0 else low 32 (VA - base))).
(*=End *)

Require Import x86proved.spred.
Lemma computeRVAsIn_interp (k: (ADDR -> program) -> program) (i:ADDR) j :
  interpProgram i j
  (k (fun VA => dd (if VA == #0 then #0 else low 32 (VA - i))))
...
(*
Lemma makeMinimalHeader_interp (i:ADDR) j  targetType numberOfSections opthdrsize :
...
(*
Lemma makePEfile_interp (i:ADDR) j
...
Definition makePEfile_program isX64
  ...
  makeRange importAddrs (fun startIAT endIAT =>
  makeRange exports (fun startEDT endEDT =>
  makePEfile dRVA isX64 targetType entry
  startEDT endEDT
  startIDT endIDT
  startIAT endIAT (* @TODO: maybe we need these? *)
  ((if exports is _::_
   then [::EDATA (makeExportSection dRVA startEDT endEDT progName exports)]
   else nil) ++
   (if imports is _::_
   then [::IDATA (makeImportSection isX64 dRVA startIDT endIDT startIAT endIAT imports importAddrs)]
   else nil) ++ sections))))) t).


(* The "false" argument to runWriter ensures that we don't emit pad bytes for alignSkip
   directives. *)
Definition assembleToString (offset:ADDR) p :=
...
Definition makeEXE base progName t :=
  assembleToString base (makePEfile_program false EXE progName t).
Definition makeDLL base progName t :=
  assembleToString base (makePEfile_program false DLL progName t).
Definition makeEXE64 base progName t :=
  assembleToString base (makePEfile_program true EXE progName t).
Definition makeDLL64 base progName t :=
  assembleToString base (makePEfile_program true DLL progName t).
