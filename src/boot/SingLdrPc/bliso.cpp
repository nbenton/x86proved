...
//  Abstract:
//
//    This module initializes and boots iSO.
//
//--
...
#define ISO_BINIMAGE_PATH  "iso.bin"
...
PVOID BlImageFileData;
UINT32 BlImageFileSize;
...

{
  ...
  //    BlPeGetVirtualRange(BlImageFileData, &BlKernelBase, &BlKernelSize);

  BlKernelSize = (BlImageFileSize / PAGE_SIZE + 1) * PAGE_SIZE;
  BlKernelBase = (PVOID) 0x300000;
  BlRtlPrintf("BL: BlImageFileSize = ... %d\n", BlImageFileSize);
  BlRtlPrintf("BL: BlKernelSize = ... %d\n", BlKernelSize);
  BlRtlPrintf("BL: BlKernelBase = ... %p\n", BlKernelBase);
  ...
  BlRtlPrintf("BL: About to copy memory\n");
  BlRtlCopyMemory(BlKernelBase,
                  BlImageFileData,
		  BlImageFileSize);

  BlRtlPrintf("BL: About to set base addr\n");
  * (PVOID *) (&BlKernelEntryPoint) = BlKernelBase;
  /*    BlPeLoadImage(BlKernelBase, BlImageFileData, (PVOID *) &BlKernelEntryPoint); */
  BlRtlPrintf("BL: About to return\n");
...

{
    ...
    if (BlFsGetFileSize(ISO_BINIMAGE_PATH, &BlImageFileSize) == FALSE) {

        BlRtlPrintf("BL: Unable to get image file!\n");
        BlRtlHalt();
    }

    BlImageFileData = BlPoolAllocateBlock(BlImageFileSize + 1);

    if (BlFsReadFile(ISO_BINIMAGE_PATH, BlImageFileData, BlImageFileSize) == FALSE) {

        BlRtlPrintf("BL: Unable to read image file!\n");
        BlRtlHalt();
    }
    ...
    BlMmDumpPhysicalRegionList();
            ...
            BlRtlPrintf("About to enter kernel\n");
    //
    // Start processor 0.
    //
